...
9.  **vTaxonRank** = The most specific level of taxonomy provided verbatim

    -   e.g. if "Stellaria media" --\> put "species"

    -   e.g. if "Stellaria" --\> put "genus"
...
11. tips about copying rows and counting how many entries needed for a given time and place - easier to copy a bunch of that info between rows

12. don't enter ones will colleciton number of "coll no \#"

13. don't enter repeated observations of same taxa at same locality , same habitat and same date

14. entry everything that could be considered an occurrence except for collected specimens (indicated with collection numbers). If certain names can't be deciphered or a location/ time can't be attributed to it, still include as a row in the template, and can be removed later (this will help for re-finding things on the page)

15. When it comes to big chunks where most observations associated with collections - only write down ones that aren't a collection at that locality - be careful it can sometimes be tricky, but can write down corodinates for collected specimen for the occurrence
