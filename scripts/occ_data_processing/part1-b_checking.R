...
date <- "2022-11-02"
requiredPackages <-  c("assertr","expss", "readxl","dplyr","here", 
                       "tidyverse","tidyr")
...
## LOADING DATA ----
  ...
  # USER INPUT: input file name for checked data & journal number
...
## CONSOLIDATNG ----
  ...
  checked_data$numPage[i] == raw_data$numPage),] <- 
    ...
    checked_data[i,2:dim(checked_data)[2]]
  }
  
## WRITING CHECKED FILE FOR NEXT PROCESSING STEP ----
  write.csv(raw_data, here::here("data", "digitized_data", 
                                 "occurrence_data","data_cleaning",
                                 paste0("checked-data_",Sys.Date(),".csv")))
