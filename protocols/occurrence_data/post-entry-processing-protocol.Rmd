...
4.  Input which taxonomy reference system you would like to use
...
6.  Run the rest of the taxonomy section until step 5 (Georeferencing)

7.  **Run the initial part of step 5 (Georeferencing)** up to an including the write.csv command . Then follow the following instructions for georeferencing in GEOlocate:
...
8.  Return to the script and run the rest of section 5 starting at the "loading georeferenced occurrences" step.

9.  A map should appear with the location of each row. If some coordinates seem off, these can be searched by referencing the coordinates indicated on the map and matching to rows by either manually searching data or using statements such as...
