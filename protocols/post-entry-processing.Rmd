...
4.  **Run the initial part of step 5 (Georeferencing)** up to an including the write.csv command . Then follow the following instructions for georeferencing in GEOlocate:
...
6.  A map should appear with the location of each row. If some coordinates seem off, these can be searched by referencing the coordinates indicated on the map and matching to rows by either manually searching data or using statements such as...
...
-   Delete older large files that have been updated, to save computer space. As long as most recent iteration of each file is kept for next data processing batch
...
-   Follow the script **"dwc-to-rbcm-format.R"** located in the folder *"scripts \> rbcm_data_processing"*

    -   This script will convert the collection data for a given book into
